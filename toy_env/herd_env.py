...
class HerdEnv(py_environment.PyEnvironment):
    """
    ...
    def action_spec(self):
        # For each heard a Booolean whether the herd is to be culled
        # encoded into a single integer as the DQN agent requires scalar discrete actions.
        num_actions = 2**self._num_herds
        return BoundedArraySpec((), dtype=np.int32,
                                minimum=0, maximum=num_actions-1, name="action")
    ...
    def _action_array(self, action:int):
        """ get the Boolean array of actions per herd from the one integer-encoded action"""
        action_array = np.zeros(self._num_herds, np.int32)
        for i in range (0, self._num_herds):
            if action & 2**i: # bit-wise AND
                action_array[i] = 1
        return action_array
