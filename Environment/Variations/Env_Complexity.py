...
Idea: Differentiate between grower and feeder herds, if necessary with differing infection dynamics
      For now: Even herds are growers, odd herds feeders. transfers from grower to feeder only
...
class Env_Comp(py_environment.PyEnvironment):
    def __init__(self,
                num_herds = 10,
    ...
    def _check_values(self):
        ...
        for j in range (0, self._num_herds):
            assert 0 <= self._state[j+self._num_herds] <= self._state[j], "Infectious go below 0 or above S."
        return True
    ...
    def _transfer(self, origin_herd = -1, target_herd = -1):
        ...
        if origin_herd >= 0 and target_herd >=0 and self._time % self._weeks_until_exchange == 0:
            ...
            infected_transfers = min(infected_transfers, self._state[origin_herd+self._num_herds])
    ...
    def _model(self, action: np.ndarray):
        ...
        def f(S, I): # Susceptible, Infected
            ...
            new_I = min(max(0, new_I), herd_population)
            return new_I
