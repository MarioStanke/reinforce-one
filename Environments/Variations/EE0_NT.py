...
Variant EE0_NT with no test cost and infectious numbers output instead of test results 
'''
import numpy as np
import os
import random
...
class EE0_NT(py_environment.PyEnvironment):
    def __init__(self,
                 num_herds = 2,
                 total_population = 300,
                 infection_rates = [0.05, 0.05],
                 exchanged_members = 0.05,    
        ...
        self._c_tests = 0.   #cost for each test 0.05 
        self._c_prime_tests = 0.   #organizational costs tests for each herd
        self._cost_removed = 1.   #herd replacement cost
    ...
    def _step(self, action: np.ndarray):
        ...
        for a in range (0, self._num_herds):
            rounded = np.rint(action[a]*self._state[a])
            action[a] = max(0, min(self._state[a], rounded))
        ...
        # Implement 'No Testing'
        for i in range (0, self._num_herds):
            g = (i*4)
            self._observation[g+1] = 1.
            self._observation[g+2] = (self._state[i+self._num_herds] / self._state[i])
